...
```powershell
git clone --recurse-submodules https://github.com/anosu/MuvluvMod.git
cd MuvluvMod
...
默认使用 `shared/Utility` 子模块中固定提交的源码，通过 `ProjectReference` 编译并复制 `Utility.dll`。PC 项目不依赖 Android Adapter。
...
```powershell
dotnet build MuvluvMod/MuvluvMod.csproj -c Release
...
保留原有输出位置：`$(GameDir)/BepInEx/plugins/MuvluvMod/<Configuration>/net6.0/`。其中包含 Mod 和本次编译的 Utility DLL。

验证构建而不写入游戏目录时：

```powershell
dotnet build MuvluvMod/MuvluvMod.csproj -c Release -p:BaseOutputPath=../artifacts/build-check/
```
...
打开生成的 `MuvluvMod.local.slnx`（需要 VS 2022 17.14 或更新版本）。它包含实际引用的共享项目，忽略 Git 追踪；修改共享项目路径后重新运行该命令。不要只向标准解决方案添加本机路径后提交。
